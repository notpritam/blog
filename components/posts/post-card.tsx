...
export function PostCard({ post, settings, headingLevel = 'h3' }: { post: PostSummary; settings: Settings; headingLevel?: 'h2' | 'h3' }) {
  ...
  return (
    <article className="post-card flex h-full flex-col py-10 md:px-8">
      {post.coverPath && (
        <Link href={`/${post.slug}`} aria-hidden="true" tabIndex={-1} className="post-card-cover mb-6 block">
          <Image src={post.coverPath} alt={post.coverAlt} width={604} height={400} sizes="(min-width: 1440px) 650px, (min-width: 768px) 46vw, 100vw" className="aspect-[1.5] w-full object-cover" />
        </Link>
      )}
